...
export interface UseMicrophoneTranscriptionProps {
  recogLang?: string
...
export function useMicrophoneTranscription(props: UseMicrophoneTranscriptionProps = {}) {
  ...
  // Gerenciar início e parada da transcrição do microfone
  useEffect(() => {
    if (enabled && browserSupportsSpeechRecognition) {
      try {
        ...
        console.error('Erro ao iniciar transcrição do microfone:', err)
        setError('Erro ao iniciar reconhecimento de fala do microfone')
      }
    } else if (!enabled) {
    ...
    if (enabled && !listening && browserSupportsSpeechRecognition && navigator.onLine) {
      const reconnectTimer = setTimeout(() => {
        try {
          ...
          console.error('Erro na reconexão do microfone:', err)
          setError('Erro na reconexão automática do microfone')
        }
      }, 1000)
