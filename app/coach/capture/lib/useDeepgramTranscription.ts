...
interface TranscriptionState {
  ...
  isForceFinalizingActive: boolean;
  provider: string;
  model: string;
  stats: {
    duration: number;
...
export const useDeepgramTranscription = () => {
  ...
  // Conectar ao servidor WebSocket Deepgram
  const connectWebSocket = useCallback(() => {
    try {
      const ws = new WebSocket('ws://localhost:8080');
      
      ws.onopen = () => {
        console.log('🔗 Conectado ao servidor Deepgram Speech-to-Text');
      ...
      ws.onmessage = (event) => {
        try {
          ...
          switch (data.type) {
            case 'connected':
              console.log('✅ Confirmação de conexão Deepgram recebida');
              setState(prev => ({ 
                ...prev, 
                isConnected: true, 
                error: null,
            ...
            case 'deepgram_connected':
              console.log('✅ Conexão Deepgram estabelecida');
              break;
              
            case 'interim':
              console.log('📝 Transcrição interim Deepgram:', data.transcript);
              setState(prev => ({
                ...prev,
                ...
                stats: {
                  ...prev.stats,
                  interimResults: prev.stats.interimResults + 1
                }
              }));
              break;
              
            case 'final':
              console.log('✅ Transcrição final Deepgram:', data.transcript);
              setState(prev => ({
                ...prev,
                ...
                stats: {
                  ...prev.stats,
                  finalResults: prev.stats.finalResults + 1
                }
              }));
              break;
              
            case 'started':
              console.log('▶️ Transcrição confirmada pelo servidor Deepgram');
              setState(prev => ({ ...prev, isListening: true }));
              break;
              
            case 'stopped':
              console.log('⏹️ Transcrição parada pelo servidor Deepgram');
              setState(prev => ({ ...prev, isListening: false }));
              break;
              
            case 'error':
              console.log('❌ Erro recebido do servidor Deepgram:', data.message);
              setState(prev => ({ 
                ...prev, 
                error: data.message,
                stats: {
            ...
            case 'force-finalize-started':
              console.log('🧠 Finalização forçada iniciada pelo servidor Deepgram');
              break;
      ...
      ws.onerror = (error) => {
        console.error('❌ Erro WebSocket Deepgram:', error);
        setState(prev => ({ 
          ...prev, 
      ...
      ws.onclose = () => {
        console.log('🔌 Conexão WebSocket Deepgram fechada');
        setState(prev => ({ 
          ...prev, 
          isConnected: false, 
      ...
      setState(prev => ({ 
        ...prev, 
        error: 'Erro ao conectar com servidor Deepgram' 
      }));
      return null;
    }
  }, []);
  ...
  const setupAudioCapture = useCallback(async () => {
    try {
      console.log('🎙️ Configurando captura de áudio para Deepgram...');
      ...
      micGain.gain.value = state.isMicrophoneEnabled ? 1 : 0;
      ...
      processor.onaudioprocess = (event) => {
        if (wsRef.current?.readyState === WebSocket.OPEN) {
          ...
          setState(prev => ({
            ...
            stats: {
              ...prev.stats,
              audioChunks: prev.stats.audioChunks + 1
            }
          }));
        }
      };
      ...
      console.log('🎙️ Captura de áudio Deepgram configurada com sucesso');
  ...
  // Iniciar transcrição (compatível com Google Cloud)
  const startListening = useCallback(async () => {
    ...
    try {
      ...
      setState(prev => ({ 
        ...prev, 
        isListening: true,
        userIsTranscribing: true,
        stats: {
          ...prev.stats,
          duration: Date.now()
        }
      }));
      console.log('🚀 Transcrição Deepgram iniciada');
    } catch (error) {
  ...
  // Parar transcrição (compatível com Google Cloud)
  const stopListening = useCallback(() => {
    try {
      ...
      setState(prev => ({ 
        ...
        stats: {
          ...prev.stats,
          duration: prev.stats.duration > 0 ? Date.now() - prev.stats.duration : 0
        }
      }));
      
      console.log('⏹️ Transcrição Deepgram parada');
    } catch (error) {
      console.error('❌ Erro ao parar transcrição:', error);
    }
  }, []);

  // Limpar transcrição (compatível com Google Cloud)
  ...
  // Forçar finalização de transcrições interim para análise (compatível com Google Cloud)
  const forceFinalize = useCallback(() => {
    console.log('🔍 forceFinalize chamado para Deepgram - verificando condições...');
    ...
    console.log('🧠 Enviando comando force-finalize para servidor Deepgram');
    ...
    return new Promise<boolean>((resolve, reject) => {
      ...
      if (wsRef.current) {
        ...
        console.log('📤 Enviando comando para servidor Deepgram:', comando);
